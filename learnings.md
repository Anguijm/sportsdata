...
### sprint-10.22-debt-sweep + gemini-council (2026-04-29/30) — PRs #57–#65

Sprint 10.22: mass debt resolution sweep (13 debts closed) + Gemini-powered automated council setup.

- **KEEP**: **Branching a new feature branch FROM another feature branch drags unrelated commits into the PR diff.** `claude/gemini-council-automation` was created from `claude/debt-16-position-weighted-injury` (not from `origin/main`), so its diff vs main included both the council automation files AND the debt-16 injury code. Council correctly flagged the debt-16 changes as problematic (it thought the `position` DB column was missing — actually 100% populated). Fix: always `git checkout -b new-branch origin/main` for independent work. Commit `f7f2788` (wrong) → `435ea72` (clean cherry-pick onto main).

- **KEEP**: **`workflow_dispatch` does not provide PR context, so the council comment step silently skips.** When we manually triggered `gh workflow run council.yml` to retry 503s, the run completed but posted no PR comment — the step gates on `$PR` being non-empty, which is only true for `pull_request` events. Correct retry mechanism: push an empty commit (`git commit --allow-empty`) to fire the `synchronize` event. Added to session knowledge.

- **KEEP**: **Position coverage in `player_stats` is 100% for all 6 sports — no migration needed for debt #16.** Before implementing position-weighted injury multipliers, queried `SELECT sport, COUNT(*), SUM(position IS NOT NULL)` — all 6054 rows across nba/nfl/mlb/nhl/mls/epl have non-null positions. The council's FAIL verdict on the contaminated PR (which flagged a "missing DB migration") was a false alarm. Verified before coding. Commit `c9bcfdc`.

- **IMPROVE**: **Council persona abstain rules must be declared for every expert, not just Math.** The Math expert already had "abstains on non-calculation reviews." Without the same rule, prediction-accuracy applied backtesting/calibration criteria to a CI/CD workflow PR and issued a spurious WARN/FAIL. Fixed by adding an explicit abstain block to `prediction-accuracy.md` (commit `76e231c`). Pattern: when adding a new council persona, always ask "when should this expert abstain?" and codify it in the file.

- **IMPROVE**: **council.py's 1-retry budget (MAX_RETRIES=1) is insufficient during Gemini API demand spikes.** Three consecutive council runs on PR #65 hit 503 UNAVAILABLE on 4 of 5 persona calls each time. A single retry with 2-second backoff is too short during sustained overload. Options: raise MAX_RETRIES to 2-3 (exponential backoff: 2s, 4s, 8s = 14s max wait), or add jitter. Deferred to a follow-up debt since it's operational, not blocking.

- **INSIGHT**: **Gemini council correctly differentiates concept CLEAR from implementation FAIL on debt #16.** The contaminated PR review (PR #64, `f7f2788`) gave Domain Expert CLEAR (position weighting is domain-correct) while Data Quality FAIL (position column thought missing) — exactly the right split. The council's hard rule "DQ FAIL = overall FAIL" correctly blocked a conceptually sound but (ostensibly) broken implementation. The system works as designed when given the right diff.

- **COUNCIL**: PR #65 (gemini-council-clean) — Run 1: WARN 8/10 (Math flagged "weighted average" language, Pred got 503). Run 2: FAIL (most personas 503 — incomplete). Run 3: FAIL (DQ CLEAR only, lead architect 503). Run 4: CLEAR 10/10 (DQ=10, Domain=10, Stats=10; Math+Pred abstained on infra PR). Merged at `a696d43`. All 9 debt branches triggered council post-merge via empty commits.
