...
### phase4-bpm-cold-start-prior (2026-04-28) — `claude/phase3-step6-calibration`

`ml/nba/bpm_prior.py` + `ml/nba/features.py` (+bpm_effective) + `ml/nba/calibrate_k.py` + `ml/nba/evaluate_cold_start.py`. BPM cold-start prior using prior-year MP-weighted BPM, K=10 (calibrated on 2022-2024 holdout). Test fold: 2025-regular, n=1162 games. Null result: Ship Rule 1 FAIL.

- **KEEP**: **Prior-year BPM does not reliably improve cold-start game prediction (games 1-20) in a high-roster-churn season.** The cold-start prior (bpm_effective feature) produced Brier delta +0.004781 (model WORSE than v5) with CI [-0.010, +0.020] spanning zero. Three domain-identified reasons: (1) 2025-26 had high roster churn, making prior-year BPM stale for many teams; (2) traded-player exclusion removed ~13-14% of active players, disproportionately the high-BPM players who drive team quality; (3) coaching_factor=0.70 was pre-declared but not applied in v1, inflating the prior's confidence in misleading directions. All three of these were known pre-experiment limitations; the null result is the expected outcome in hindsight.
- **KEEP**: **v5's base-rate fallback (0.57 for <5 games) is a surprisingly strong cold-start handler.** For games 1-4, v5 returns flat 0.57 — no team-specific signal at all. This outperformed the BPM prior on early-season games because a calibrated flat prior beats a noisy team-specific prior when team-specific information is unreliable. The lesson: a simple, stable fallback can beat a sophisticated but noisy prior if the prior's signal quality is low.
- **KEEP**: **`N-regular` season labels in this codebase mean the season STARTING in year N (Oct N – Apr N+1).** bbref year N = the N-1/N season. So `2025-regular` = Oct 2025–Apr 2026; bbref 2026 = the 2025-26 season. When computing priors from prior-year BPM: for a game in `2025-regular`, load bbref year 2025 (= 2024-25 season BPM). The `calibrate_k.py` bug fixed in this sprint had `prior_year = season_year - 1` (wrong → bbref 2024 = 2023-24 season) instead of `prior_year = season_year` (correct → bbref 2025 = 2024-25 season). The convention is non-obvious and has caused multiple bugs; this entry is the authoritative record.
- **KEEP**: **Block-bootstrap CI width scales as ~σ/√n × √(effective n / actual n), where effective n is reduced by within-block correlation.** For n=309 early-season games with ~150-180 unique (home_team × ISO-week) blocks, the 95% CI width of ~0.030 Brier units is correct and honest — it is not a methodology failure. A CI this wide means the experiment was underpowered for effects < 0.005 Brier. Future cold-start experiments need ≥2 test seasons (~600+ early-season games) to have power to detect the target effect size.
- **KEEP**: **bbref multi-team rows (2TM/3TM/4TM) must be detected by testing `len(abbr)==3 and abbr[0].isdigit() and abbr.endswith("TM")`, not just `== "TOT"`.** The original `calibrate_k.py` used `if team == "TOT"`, which fails for 3TM/4TM rows. The correct check is the length+digit+TM suffix pattern. These multi-team rows represent traded players for whom no single team assignment exists; v1 excludes them entirely.
- **INSIGHT**: **The overall model (44 features, ewma-h21) is still AUC 0.7241 < v5 AUC 0.7283.** The BPM features (home_bpm_effective + away_bpm_effective) improved Phase 3 AUC from 0.6954 to 0.7241 (+0.029) — a significant jump from the 42-feature unfeaturized baseline. But v5 still leads. The gap is now small (~0.004 AUC) suggesting the remaining delta is in the earliest-season games where BPM is relevant but the prior quality is insufficient. Fixing the three identified v2 prerequisites may close the gap.
- **COUNCIL**: Gate 1 CLEAR (avg 8.2/10, round 2). Gate 2 WARN→CLEAR (avg 8.0/10 after 3 documentation fixes). Gate 3 (K calibration results) CLEAR (avg 8.0/10). Gate 4 (test-fold results) CLEAR (avg 7.6/10). Null result recorded.

### phase5-tov-fix-plus-reg-season-val (2026-04-29) — `claude/nba-cold-start-prior-plan`

`ml/nba/features.py` (TOV% fraction fix) + `ml/nba/cv_runner.py` + `ml/nba/calibrate.py` (regular-season val fold filter). Two genuine bug fixes; both correctly implemented. Null result: Gate D FAIL (AUC 0.7221 < v5 0.7283). Phase 5 closed.

- **KEEP**: **Fixing real bugs in a model does not guarantee test-fold improvement.** The TOV% fix recovered 4 features of real signal (tov_pct_off/def, home/away) from effective zero. The val fold fix removed postseason bias from calibration and Brier reporting. Both were genuine fixes. The AUC gap to v5 barely moved (0.7241 → 0.7221 — slightly worse, not better). Moral: when a model fails a gate multiple times at a consistent gap, the problem is structural, not fixable by bug patches. Identify the root cause before writing the next plan.
- **KEEP**: **A val Brier improvement that doesn't appear at test time is a red flag for optimistic bias removal, not signal improvement.** Phase 5 val Brier improved significantly (0.203 mixed-fold → 0.196 reg-season-only). Test Brier did not improve (0.211973 vs Phase 4's 0.211920 — essentially unchanged). The val improvement came entirely from removing postseason games from the metric, not from the model learning better. When val improves but test doesn't, the right question is: did we remove bias from the metric, or from the model?
- **KEEP**: **Three consecutive Gate D FAILs at the same AUC level is a structural signal, not sampling noise.** Phase 3 (42-feature EWMA): 0.6954. Phase 4 (44-feature + BPM): 0.7241. Phase 5 (44-feature + bug fixes): 0.7221. All below v5 0.7283. The gap is ~0.004–0.006 AUC and has been consistent across different feature sets, bug fixes, and model variants. This is the kind of pattern that warrants architectural change, not incremental iteration.
- **INSIGHT**: **The AUC gap to v5 is explained by a single missing feature class: season-to-date cumulative point differential.** v5 is sigmoid(0.10 × (home_net_rating − away_net_rating) + 2.25·home_indicator). `net_rating = (season pts for − season pts against) / games`. This integrates strength of schedule, home/away split, and season-long form into a single number — more stable than EWMA for team quality ranking. Adding `home_season_net_rating` and `away_season_net_rating` directly as features gives the LightGBM model access to exactly this signal, at which point the AUC gap should close. Phase 6 tests this hypothesis.
- **COUNCIL**: Gate 1 WARN→CLEAR (avg 7.6/10, fix-pack). Gate 2 CLEAR (avg 7.6/10). Both council reviews at 7.6/10 — consistent with a well-executed but structurally limited bug-fix release.

### phase6-season-aggregate (2026-04-29) — `claude/nba-cold-start-prior-plan`

`ml/nba/features.py` (+ season_net_rating). Added `home_season_net_rating` and `away_season_net_rating` (season-to-date mean Net Rating, ORtg−DRtg per 100 possessions, averaged over all current-season games before game_date). 46 features total. Gate 1 CLEAR (8.0/10). Gate 2 implementation review CLEAR (8.4/10). Null result: Gate D FAIL (AUC 0.7237 < v5 0.7283).

- **KEEP**: **Adding v5's exact signal as explicit features does not guarantee Gate D clearance when EWMA features dilute the signal.** AUC improved only +0.0016 vs Phase 5 (0.7221 → 0.7237). LightGBM with 46 features and min_child_samples=200 at n=2640 assigns split priority to the most locally discriminative splits, which are spread across 44 EWMA features competing with 2 season-aggregate features. The season-aggregate signal exists but can't dominate. Lesson: feature count and model capacity interact — adding the right features to a high-regularization model may not help if the model can't leverage them.
- **KEEP**: **Four consecutive Gate D FAILs at the same AUC level (0.722–0.724) is architectural evidence, not sampling variance.** Phase 3 (0.6954) was anomalously low — likely the TOV% zeroing bug. Phase 4 (0.7241), Phase 5 (0.7221), Phase 6 (0.7237) all cluster at essentially the same level across very different feature sets. The model ceiling at ~0.724 AUC for this architecture (EWMA + LightGBM, n=2640) is stable. Crossing v5's 0.7283 floor requires either (a) a different model architecture (linear model, explicit formula), (b) more training data (additional seasons), or (c) removing the EWMA features entirely to let the season-aggregate signal dominate.
- **INSIGHT**: **v5's architecture (fixed sigmoid formula with hand-tuned parameters) is extremely sample-efficient compared to a learned model at n=2640.** v5 encodes `sigmoid(0.10 × (home_diff − away_diff) + 2.25 × home_indicator)` — a 3-parameter model. A LightGBM with 46 features and 20 seeds has thousands of implicit parameters. At n=2640 training games, the simpler model wins. This is a fundamental tradeoff: flexibility vs. sample efficiency. Phase 7 should try a regularized logistic regression on the 46-feature set — far fewer effective parameters than LightGBM, should learn (home_season_net_rating − away_season_net_rating) efficiently.
- **INSIGHT**: **Season-to-date mean Net Rating (ORtg−DRtg per 100 possessions) is a clean analogue of v5's raw point differential per game.** They correlate at >0.99. Net Rating is slightly better analytically (pace-adjusted). Adding it to `ORDERED_STATS` + `_feature_names_for` generates `home_season_net_rating` and `away_season_net_rating` automatically and routes through `zscore` normalization. The implementation was correct and clean (15 lines); it's the model's ability to use the feature that was the bottleneck, not the feature itself.
- **COUNCIL**: Gate 1 WARN→CLEAR (avg 8.0/10, two minor fix-pack items). Gate 2 implementation review CLEAR (avg 8.4/10 — highest impl-review score so far). Gate 2 results CLEAR (avg 7.4/10). Null result recorded.
